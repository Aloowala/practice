...
class Game {
  ...
  start() {
    this.resize();
    const GH = this.canvas.height * 0.78;
  ...
  update() {
    ...
    if (this.score % 500 === 0) this.spawnInterval = Math.max(80, this.spawnInterval - 5);
  ...
  spawnWall() {
    ...
    // Jump height the player can reach (generous estimate)
    const jumpHeight = (JUMP_FORCE * JUMP_FORCE) / (2 * GRAVITY);
    // Max safe obstacle height from ground (player must be able to clear it)
    const maxObstacleH = jumpHeight * 0.72;
    // Min clearance gap for gap-walls (cube size + comfortable margin)
    const minGap = CUBE_SIZE * 3.5;

    const type = Math.random();

    if (type < 0.6) {
      // GROUND obstacle — rises from floor, player jumps OVER it
      // Height: 1 to 2 cube sizes — always jumpable
      const h = CUBE_SIZE * (1 + Math.random() * 1.2);
      this.walls.push({ x: W + 10, y: GH - h, w: WALL_W, h: h + 20 });

    } else if (type < 0.85 && this.score > 150) {
      // CEILING obstacle — hangs from top, player ducks under (stays on ground)
      // Leave at least 2.5x cube height gap from floor so player can pass
      const clearance = CUBE_SIZE * 2.8;
      const h = GH - clearance;
      if (h > 30) {
        this.walls.push({ x: W + 10, y: 0, w: WALL_W, h });
      } else {
        // fallback to ground obstacle if screen too short
        const fh = CUBE_SIZE * 1.2;
        this.walls.push({ x: W + 10, y: GH - fh, w: WALL_W, h: fh + 20 });
      }

    } else if (this.score > 300) {
      // GAP WALL — hole in the middle, player jumps through the gap
      const gap = minGap + Math.random() * CUBE_SIZE * 1.5;
      // Place gap so its bottom edge sits just above ground level
      const gapBottom = GH - CUBE_SIZE * 0.5;
      const gapTop = gapBottom - gap;
      if (gapTop > 20) {
        this.walls.push({ x: W + 10, y: 0, w: WALL_W, h: gapTop });
        this.walls.push({ x: W + 10, y: gapBottom, w: WALL_W, h: H - gapBottom });
      } else {
        const fh = CUBE_SIZE * 1.2;
        this.walls.push({ x: W + 10, y: GH - fh, w: WALL_W, h: fh + 20 });
      }
